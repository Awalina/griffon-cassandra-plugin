...
class CassandraGriffonPlugin {
    ...
    String description = '''
The Cassandra plugin enables lightweight access to [Cassandra][1] datastores using [JDBC][2] + [CQL][3]. 
...
This method is also accessible to any component through the singleton `griffon.plugins.cassandra.CassandraConnector`.
You can inject these methods to non-artifacts via metaclasses. Simply grab hold of a particular metaclass and call
`CassandraEnhancer.enhance(metaClassInstance, cassandraProviderInstance)`.

Configuration
-------------
### Dynamic method injection

The `withCql()` dynamic method will be added to controllers by default. You can
...
Testing
-------
The `withCql()` dynamic method will not be automatically injected during unit testing, because addons are simply not initialized
for this kind of tests. However you can use `CassandraEnhancer.enhance(metaClassInstance, cassandraProviderInstance)` where 
`cassandraProviderInstance` is of type `griffon.plugins.cassandra.CassandraProvider`. The contract for this interface looks like this
...
fails regardless of the arguments it receives
    ...
    class MyCassandraProvider implements CassandraProvider {
        Object withCql(String dataSourceName = 'default', Closure closure) { null }
        public <T> T withCql(String dataSourceName = 'default', CallableWithArgs<T> callable) { null }
    }
